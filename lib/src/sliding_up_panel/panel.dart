...
class CustomSlidingUpPanel extends StatefulWidget {
  ...
  /// The width of the sliding panel when fully collapsed.
  final double minWidth;

  /// The width of the sliding panel when fully open.
  final double maxWidth;

  /// A point between [minWidth] and [maxWidth] that the panel snaps to
  ...
  /// percentage of the total animation distance ([maxWidth] - [minWidth]),
...
class _CustomSlidingUpPanelState extends State<CustomSlidingUpPanel>
        ...
        widget.body != null
            ? AnimatedBuilder(
                ...
                builder: (context, child) {
                  return Positioned(
                    right: widget.parallaxEnabled ? _getParallax() : 0.0,
            ...
            : GestureDetector(
                onHorizontalDragEnd: widget.backdropTapClosesPanel
                    ? (DragEndDetails dets) {
                        ...
                        if ((widget.slideDirection == SlideDirection.LEFT ? 1 : -1) * dets.velocity.pixelsPerSecond.dx > 0) _close();
                      }
                    : null,
  ...
  double _getParallax() {
    if (widget.slideDirection == SlideDirection.LEFT)
      return -_ac.value * (widget.maxWidth - widget.minWidth) * widget.parallaxOffset;
    else
      return _ac.value * (widget.maxWidth - widget.minWidth) * widget.parallaxOffset;
  }
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    if (widget.panel != null) {
      return GestureDetector(
        onHorizontalDragUpdate: (DragUpdateDetails dets) => _onGestureSlide(dets.delta.dx * -1),
  ...
  void _onGestureEnd(Velocity v) {
    ...
    double visualVelocity = -v.pixelsPerSecond.dy / (widget.maxWidth - widget.minWidth);
...
class PanelController {
  _CustomSlidingUpPanelState? _panelState;

  void _addState(_CustomSlidingUpPanelState panelState) {
  ...
  /// Closes the sliding panel to its collapsed state (i.e. to the  minWidth)
  Future<void> close() {
