...
final homeProvider = ChangeNotifierProvider(
  ((ref) {
    ...
    homeModel.mainModel = ref.read(mainProvider);
...
class HomeModel extends ChangeNotifier {
  ...
  Future<void> onRefresh(RefreshController refreshController) async {
    final QuerySnapshot<Map<String, dynamic>> timelinesQshot = await FirebaseFirestore.instance.collection('users').doc(currentUser.uid).collection('timelines').orderBy('createdAt', descending: true).startAfterDocument(timelineDocs.first).limit(tenCount).get();
  ...
  void setVideoPlayerController() {
    // mainModel.videoPlayerController = VideoPlayerController.networkUrl(Uri.parse(imageURL));
  }
